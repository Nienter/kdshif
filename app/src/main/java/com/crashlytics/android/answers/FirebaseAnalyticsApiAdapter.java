...
class FirebaseAnalyticsApiAdapter {
    ...
    public void processEvent(SessionEvent sessionEvent) {
        ...
        if (firebaseAnalytics == null) {
            Fabric.m456h().mo8506a(Answers.TAG, "Firebase analytics logging was enabled, but not available...");
            return;
        }
        FirebaseAnalyticsEvent mapEvent = this.eventMapper.mapEvent(sessionEvent);
        if (mapEvent == null) {
            Fabric.m456h().mo8506a(Answers.TAG, "Fabric event was not mappable to Firebase event: " + sessionEvent);
            return;
        }
