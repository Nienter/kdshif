...
public final class zzjj {
    ...
    public zzjj(String str) {
        JSONObject jSONObject = new JSONObject(str);
        if (zzpe.zzai(2)) {
            String valueOf = String.valueOf(jSONObject.toString(2));
            zzpe.m2431v(valueOf.length() != 0 ? "Mediation Response JSON: ".concat(valueOf) : new String("Mediation Response JSON: "));
        }
        JSONArray jSONArray = jSONObject.getJSONArray("ad_networks");
        ArrayList arrayList = new ArrayList(jSONArray.length());
        int i = -1;
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            zzji zzji = new zzji(jSONArray.getJSONObject(i2));
            arrayList.add(zzji);
            if (i < 0 && zza(zzji)) {
                i = i2;
            }
    ...
    private boolean zza(zzji zzji) {
        for (String equals : zzji.zzJI) {
