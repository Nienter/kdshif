...
public class zzy extends Thread implements SurfaceTexture.OnFrameAvailableListener, zzx.zza {
    ...
    private EGLConfig zzhW() {
        ...
        if (!this.zzOh.eglChooseConfig(this.zzOi, new int[]{12352, 4, 12324, 8, 12323, 8, 12322, 8, 12325, 16, 12344}, eGLConfigArr, 1, iArr)) {
            return null;
        }
        if (iArr[0] > 0) {
    ...
    private String zzhX() {
        zzft<String> zzft = zzfx.zzCI;
        return !zzft.get().equals(zzft.zzfm()) ? zzft.get() : "attribute highp vec3 aPosition;varying vec3 pos;void main() {  gl_Position = vec4(aPosition, 1.0);  pos = aPosition;}";
    }

    private String zzhY() {
        zzft<String> zzft = zzfx.zzCJ;
        return !zzft.get().equals(zzft.zzfm()) ? zzft.get() : "#extension GL_OES_EGL_image_external : require\n#define INV_PI 0.3183\nprecision highp float;varying vec3 pos;uniform samplerExternalOES uSplr;uniform mat3 uVMat;uniform float uFOVx;uniform float uFOVy;void main() {  vec3 ray = vec3(pos.x * tan(uFOVx), pos.y * tan(uFOVy), -1);  ray = (uVMat * ray).xyz;  ray = normalize(ray);  vec2 texCrd = vec2(    0.5 + atan(ray.x, - ray.z) * INV_PI * 0.5, acos(ray.y) * INV_PI);  gl_FragColor = vec4(texture2D(uSplr, texCrd).xyz, 1.0);}";
    }
    ...
    public void run() {
        boolean z = true;
        if (this.zzOa == null) {
            zzpe.m2432e("SphericalVideoProcessor started with no output texture.");
            this.zzOf.countDown();
            return;
        }
        boolean zzhV = zzhV();
        ...
        if (!zzhV || !z) {
            ...
            zzpe.m2432e(concat);
            zzv.zzcN().zza(new Throwable(concat), "SphericalVideoProcessor.run.1");
            zzhZ();
            this.zzOf.countDown();
            return;
        }
    ...
    public boolean zzhZ() {
        boolean z = false;
        if (!(this.zzOk == null || this.zzOk == EGL10.EGL_NO_SURFACE)) {
            z = this.zzOh.eglMakeCurrent(this.zzOi, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_CONTEXT) | false | this.zzOh.eglDestroySurface(this.zzOi, this.zzOk);
            this.zzOk = null;
        }
        if (this.zzOj != null) {
        ...
        boolean eglTerminate = z | this.zzOh.eglTerminate(this.zzOi);
        this.zzOi = null;
        return eglTerminate;
    }
